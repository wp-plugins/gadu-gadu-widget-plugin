...
break;
case 3:
echo '<img src="'.$gimageurl.'gg-off.png" style="border: 0; vertical-align:middle" alt="Zaraz wracam" /> Zaraz wracam';
break;
default:
...
// Options form for the widget - called in the Sidebar Widgets of the Presentation tab
	...
	if ( !is_array($gaduoptions) )
        ...
        $gtitle = htmlspecialchars($gaduoptions['gtitle'], ENT_QUOTES);
	$janbr = htmlspecialchars($gaduoptions['janbr'], ENT_QUOTES); 

	// The control form for editing options.        

        echo '<p><label for="gadu-title">' . _('Title:') . ' <input style="width: 200px;" type="text" id="gadu-title" name="gadu-title" value="'.$gadutitle.'" /></label></p>';
